...
for i in range(0,lmax):
    ...
    for j in range(0,nmax):
        ...
        x5.append([t,rad])
        t=t+0.005
    mplt.plot([row[0] for row in x5], [row[1] for row in x5], colours[i], linewidth=2.0, label="L = %.3f"%(l))
    l=l+0.5
    del x4[:]
    del x5[:]
#mplt.subplot(211)
...
#mplt.xlabel(r'Absorptivity, $\alpha$')
mplt.xlabel('Thickness, t [m]')
mplt.ylabel("Direct Radiative Power, $I_L$ [W]")
mplt.axis([0.005,0.06,0,2000])
mplt.legend()
mplt.show()
